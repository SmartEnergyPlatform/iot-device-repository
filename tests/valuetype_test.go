...
func TestValuetypeDeleteSideeffect(t *testing.T) {
   ...
   "vendor":{  
	...
	vt = model.ValueType{}
	Jwtuser.GetJSON("http://localhost:"+util.Config.ServerPort+"/valueType/iot%23e69373a9-2ab9-4dc4-b5d5-ff57aa742c3e", &vt)
	if vt.Name != "" || len(vt.Fields) != 0 {
		t.Fatal(err, vt)
	}
	...
	vt = model.ValueType{}
	Jwtuser.GetJSON("http://localhost:"+util.Config.ServerPort+"/valueType/iot%23e9104f3f-ffe1-410a-befa-dd68f0677ec6", &vt)
	if vt.Name != "" || len(vt.Fields) != 0 {
		t.Fatal(err, vt)
	}
}
